...
library(knitr)
opts_chunk$set(include = TRUE, warning = FALSE, message = FALSE)
```

```{r knitting, echo=TRUE, eval=FALSE}
rmarkdown::render("Hinic_HW2_pdf.Rmd",
...
```{r dplyr pic, echo=FALSE, eval=TRUE, out.width="70px", fig.align='center'}
...
These all combine naturally with `group_by()` which allows you to perform any operation "by group". You can learn more about them in vignette("dplyr"). As well as these single-table verbs, dplyr also provides a variety of two-table verbs, which you can learn in `vignette("two-table")`.

If you are new to dplyr, the best place to start is the data transformation chapter in R for data science.

```{r dplyr, echo=TRUE, eval=TRUE}
library(dplyr)
...
```{r ggplot2 pic, echo=FALSE, eval=TRUE, out.width="70px", fig.align='center'}
...
```{r readr pic, echo=FALSE, eval=TRUE, out.width="70px", fig.align='center'}
...
```{r tidyr pic, echo=FALSE, eval=TRUE, out.width="70px", fig.align='center'}
...
```{r tidyr 1, echo=TRUE, eval=TRUE}
library(tidyr)

relig_income
```

```{r tidyr 2, echo=TRUE, eval=TRUE}
relig_income %>%
